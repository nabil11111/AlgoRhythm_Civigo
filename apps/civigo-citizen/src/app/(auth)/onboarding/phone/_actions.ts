...
export async function sendOtp(
  ...
  // Prevent using a phone already bound to a different existing profile
  ...
  if (existingProfile?.id) {
    return { ok: false, error: "phone_in_use" } as const;
  }
  ...
  if ((c1 ?? 0) > 0)
    return { ok: false, error: "rate_limited_minute" } as const;
  ...
  await supabase
    .from("phone_verification_events")
    .insert({ user_temp_id: tempId, phone });
...
export async function verifyOtp(
  ...
  if (new Date(pv.expires_at).getTime() < Date.now())
