...
const schema = z.object({
  front_path: z.string().min(1),
  back_path: z.string().min(1),
});

export async function saveNicPhotos(
  ...
  const parsed = schema.safeParse({
    front_path: formData.get("front_path"),
    back_path: formData.get("back_path"),
  });
...
export async function getNicUploadUrl(
  prev: { ok: boolean; error?: string; url?: string } | null,
  formData: FormData
) {
  const kind = String(formData.get("kind") || "");
  if (kind !== "front" && kind !== "back")
    return { ok: false, error: "invalid_kind" } as const;
  const cookieStore = await cookies();
  const tempId = cookieStore.get("onboarding_temp_id")?.value;
  if (!tempId) return { ok: false, error: "no_session" } as const;
  const supabase = getServiceRoleClient();
...
export async function uploadNicPhoto(
...
export async function getSignedNicPhotoUrl(
  prev: { ok: boolean; error?: string; url?: string } | null,
  formData: FormData
) {
  const kind = String(formData.get("kind") || "");
  ...
  if (error || !signed?.signedUrl)
    return { ok: false, error: "sign_failed" } as const;
