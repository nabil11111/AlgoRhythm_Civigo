  ...
  return (
    <form onSubmit={onSave} className="space-y-3">
      <div className="flex items-center gap-2">
        ...
        <Button type="button" onClick={() => getUrl("front")}>
          Get URL
        </Button>
        <input
          type="file"
          accept="image/*"
          onChange={(e) => onUpload("front", e.target.files?.[0] || null)}
        />
      </div>
      <div className="flex items-center gap-2">
        <div className="flex-1">
          ...
          <Input
            value={backPath}
            onChange={(e) => setBackPath(e.target.value)}
          />
        </div>
        <Button type="button" onClick={() => getUrl("back")}>
          Get URL
        </Button>
        <input
          type="file"
          accept="image/*"
          onChange={(e) => onUpload("back", e.target.files?.[0] || null)}
        />
      </div>
      <div>
