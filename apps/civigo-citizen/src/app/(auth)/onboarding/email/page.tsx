    ...
    <div className="min-h-[100svh] w-full flex flex-col bg-white">
      ...
      <div className="w-full border-b-[6px] border-b-[var(--color-primary)] rounded-b-2xl shadow-[0_8px_24px_rgba(0,82,165,0.18)] bg-white">
        <div className="flex items-center justify-between px-4 pt-4">
          <a href="/onboarding/phone/verify" aria-label="Back" className="inline-flex h-10 w-10 items-center justify-center rounded-full border-2 border-[var(--color-primary)] text-[var(--color-primary)]">←</a>
          <div className="h-1.5 w-[147px] rounded bg-[#787878]/20">
            <div className="h-1.5 rounded bg-[var(--color-secondary)]" style={{ width: "42%" }} />
          </div>
      ...
      <div className="mx-auto w-full max-w-[360px] px-4 mt-10 pb-40">
        {errorMessage && (
          <div role="alert" aria-live="polite" className="rounded border border-red-200 bg-red-50 text-red-800 text-sm px-3 py-2 mb-4">
            {errorMessage}
          </div>
        )}
        <form action={submitEmail} className="space-y-6">
          <div className="text-center">
            <h2 className="text-xl font-semibold text-[#4f4f4f]">Enter Your Email</h2>
          </div>
          <div>
          ...
          <div className="fixed inset-x-0 bottom-0 z-50 bg-white/95 backdrop-blur supports-[backdrop-filter]:bg-white/80 px-4 pb-[calc(env(safe-area-inset-bottom,0)+16px)] pt-2">
            <button type="submit" className="w-full rounded-md bg-[var(--color-primary)] text-white py-3.5 text-[18px] font-medium">
              Continue
            </button>
          </div>
        </form>
