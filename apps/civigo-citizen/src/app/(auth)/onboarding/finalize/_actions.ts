...
export async function finalizeOnboarding(
  ...
  if (!idv.nic_front_path || !idv.nic_back_path || !idv.face_capture_path)
    return { ok: false, error: "media_missing" } as const;
  const namesCookie = (await cookies()).get("onboarding_names")?.value;
  const password = (await cookies()).get("onboarding_password")?.value;
  if (!namesCookie || !password)
    return { ok: false, error: "incomplete" } as const;
  ...
  // Create auth user with provided email (no verification) and password
  const email =
    (await cookies()).get("onboarding_email")?.value ??
    `${govId}@placeholder.local`;

  // Pull latest verified phone (if present) and persist to profile
  const { data: phoneRow } = await supabase
    ...
    .limit(1)
    .maybeSingle();
  const { error: authErr, data: signUpData } =
  ...
  await supabase
    ...
    .update({
      ...
      phone: phoneRow?.verified_at ? phoneRow?.phone ?? null : null,
    })
    .eq("id", profileId);
  ...
  cookieStore.set("onboarding_names", "", {
    httpOnly: true,
    sameSite: "lax",
    secure: true,
    path: "/",
  ...
  cookieStore.set("onboarding_password", "", {
    httpOnly: true,
    sameSite: "lax",
    secure: true,
    path: "/",
