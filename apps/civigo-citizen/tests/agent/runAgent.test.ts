...
vi.mock("@/app/(protected)/_agent_tools", () => ({
  getUserAppointments: vi.fn(async () => [
    {
      id: "a1",
  ...
  getServiceInstructions: vi.fn(async () => ({
    serviceName: "Passport",
    branches: [],
    instructions_richtext: {},
    instructions_pdf_url: undefined,
  })),
...
describe("runAgent", () => {
  ...
  it("answers slots", async () => {
    ...
    const res = await runAgent("find slots", {
      serviceId: "svc",
      dateFromISO: from,
      dateToISO: to,
    });
  ...
  it("answers documents/instructions", async () => {
    const res = await runAgent("What documents or instructions are needed?", {
      serviceId: "svc",
    });
